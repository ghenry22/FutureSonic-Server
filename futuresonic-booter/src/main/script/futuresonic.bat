...
REM  The directory where FutureSonic will create files. Make sure it is writable.
set MADSONIC_HOME=c:\futuresonic

REM  The host name or IP address on which to bind FutureSonic. Only relevant if you have
REM  multiple network interfaces and want to make FutureSonic available on only one of them.
REM  The default value 0.0.0.0 will bind FutureSonic to all available network interfaces.
set MADSONIC_HOST=0.0.0.0

REM  The port on which FutureSonic will listen for incoming HTTP traffic.
set MADSONIC_PORT=4040

REM  The port on which FutureSonic will listen for incoming HTTPS traffic (0 to disable).
set MADSONIC_HTTPS_PORT=0

REM  The context path (i.e., the last part of the FutureSonic URL).  Typically "/" or "/futuresonic".
