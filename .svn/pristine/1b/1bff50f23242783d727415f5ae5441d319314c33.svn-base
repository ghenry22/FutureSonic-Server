...
Public Class Plugin
    ...
    Public Function Initialise(ByVal apiInterfacePtr As IntPtr) As PluginInfo
        ...
        FutureSonic.SendNotificationHandler = mbApiInterface.MB_SendNotification
        about.PluginInfoVersion = PluginInfoVersion
        about.Name = "FutureSonic"
        about.Description = "Access files and playlists on a FutureSonic Server"
        about.Author = "Madevil"
        about.TargetApplication = "FutureSonic"
    ...
    Public Function Configure(ByVal panelHandle As IntPtr) As Boolean
        If panelHandle <> IntPtr.Zero Then
            ...
            basePath.Text = FutureSonic.BasePath
            ...
            username.Text = FutureSonic.Username
    ...
    Public Sub ReceiveNotification(ByVal sourceFileUrl As String, ByVal type As NotificationType)
        If type = NotificationType.PluginStartup Then
            FutureSonic.CacheUrl = mbApiInterface.Setting_GetPersistentStoragePath() & "\futuresonicCache.dat"
            FutureSonic.SettingsUrl = mbApiInterface.Setting_GetPersistentStoragePath() & "\futuresonicSettings.dat"
            If FutureSonic.Initialise() Then
                FutureSonic.SendNotificationHandler.Invoke(CallBackType.StorageReady)
            Else
                FutureSonic.SendNotificationHandler.Invoke(CallBackType.StorageFailed)
            End If
        End If
    End Sub

    Public Sub SaveSettings()
        If Not FutureSonic.SetHost(host.Text.Trim(), port.Text.Trim(), basePath.Text.Trim(), username.Text.Trim(), password.Text.Trim(), transcode.Checked) Then
            Dim message As String = FutureSonic.GetError().Message
            If message IsNot Nothing Then
                Windows.Forms.MessageBox.Show(host, "Error: " & message & "     ", "FutureSonic Plugin", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            End If
        End If
    End Sub

    Public Sub Refresh()
        If Not FutureSonic.IsInitialised Then
            If FutureSonic.Initialise() Then
                FutureSonic.SendNotificationHandler.Invoke(CallbackType.StorageReady)
            Else
                FutureSonic.SendNotificationHandler.Invoke(CallbackType.StorageFailed)
    ...
    Public Function IsReady() As Boolean
        Return FutureSonic.IsInitialised
    End Function
        ...
        Public Shared Function SetHost(ByVal host As String, ByVal port As String, ByVal basePath As String, ByVal username As String, ByVal password As String, ByVal transcode As Boolean) As Boolean
            ...
            Try
                ...
                If Not basePath.EndsWith("/"c) Then
                    basePath &= "/"c
                End If
                Dim isChanged As Boolean = (host <> FutureSonic.Host OrElse port <> FutureSonic.Port OrElse basePath <> FutureSonic.BasePath OrElse username <> FutureSonic.Username OrElse password <> FutureSonic.Password)
                If isChanged Then
                    Dim savedHost As String = FutureSonic.Host
                    Dim savedport As String = FutureSonic.Port
                    Dim savedBasePath As String = FutureSonic.BasePath
                    Dim savedUsername As String = FutureSonic.Username
                    Dim savedPassword As String = FutureSonic.Password
                    Dim isPingOk As Boolean
                    Try
                        FutureSonic.Host = host
                        FutureSonic.Port = port
                        FutureSonic.BasePath = basePath
                        FutureSonic.Username = username
                        FutureSonic.Password = password
                        isPingOk = SetServerName()
                    Catch
                        isPingOk = False
                    End Try
                    If Not isPingOk Then
                        FutureSonic.Host = savedHost
                        FutureSonic.Port = savedport
                        FutureSonic.BasePath = savedBasePath
                        FutureSonic.Username = savedUsername
                        FutureSonic.Password = savedPassword
                        Return False
                    End If
                    IsInitialised = True
                End If
                isChanged = (isChanged OrElse transcode <> FutureSonic.Transcode)
                ...
                End Using
                FutureSonic.Transcode = transcode
                Try
