...
public class VersionService {
     ...
     */
    private static final String VERSION_URL = "http://futuresonic.googlecode.com/svn/version.html";
    ...
    public synchronized Version getLocalVersion() {
        if (localVersion == null) {
            try {
                ...
                LOG.info("Resolved local FutureSonic version to: " + localVersion);
            } catch (Exception x) {
                LOG.warn("Failed to resolve local FutureSonic version.", x);
            }
        }
        return localVersion;
    }
    ...
    private void refreshLatestVersion(boolean beta) {
        ...
        if (isBetaOutdated || isOutdated) {
            try {
                ...
                LOG.warn("## Failed to resolve latest FutureSonic version.", x);
            }
        }
    }
        ...
        try {
            ...
            while (line != null) {
                ...
                if (finalMatcher.find()) {
                    ...
                    LOG.info("Resolved latest FutureSonic final version to: " + latestFinalVersion);
                }
                Matcher betaMatcher = betaPattern.matcher(line);
                if (betaMatcher.find()) {
                    latestBetaVersion = new Version(betaMatcher.group(1));
                    LOG.info("Resolved latest FutureSonic beta version to: " + latestBetaVersion);
                }
