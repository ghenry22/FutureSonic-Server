...
/**
 * Responsible for deploying the FutureSonic web app in
 * the embedded Jetty container.
 * <p/>
 * The following system properties may be used to customize the behaviour:
 * <ul>
 * <li><code>subsonic.contextPath</code> - The context path at which FutureSonic is deployed.  Default "/".</li>
 * <li><code>subsonic.port</code> - The port FutureSonic will listen to.  Default 4040.</li>
 * <li><code>subsonic.httpsPort</code> - The port FutureSonic will listen to for HTTPS.  Default 0, which disables HTTPS.</li>
 * <li><code>futuresonic.war</code> - FutureSonic WAR file, or exploded directory.  Default "futuresonic.war".</li>
 * <li><code>subsonic.createLinkFile</code> - If set to "true", a FutureSonic.url file is created in the working directory.</li>
...
public class SubsonicDeployer implements SubsonicDeployerService {
    ...
    public static final String DEFAULT_WAR = "futuresonic.war";
    ...
    private static final File SUBSONIC_HOME_WINDOWS = new File("c:/futuresonic");
    private static final File SUBSONIC_HOME_OTHER = new File("/var/futuresonic");
    ...
    private void createLinkFile() {
        if ("true".equals(System.getProperty("subsonic.createLinkFile"))) {
            Writer writer = null;
            try {
                writer = new FileWriter("futuresonic.url");
                writer.append("[InternetShortcut]");
                writer.append(System.getProperty("line.separator"));
                writer.append("URL=").append(getUrl());
                writer.flush();
            } catch (Throwable x) {
                System.err.println("Failed to create futuresonic.url.");
                x.printStackTrace();
            } finally {
                if (writer != null) {
                    try {
    ...
    private String getWar() {
        String war = System.getProperty("futuresonic.war");
        if (war == null) {
            war = DEFAULT_WAR;
        }
    ...
    /**
     * Returns the FutureSonic home directory.
     *
     * @return The FutureSonic home directory, if it exists.
